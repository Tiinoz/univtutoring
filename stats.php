...
if(has_capability('mod/univtutoring:view_stats',$context)){
    echo $OUTPUT->heading(get_string('stats','mod_univtutoring'));
    $context = context_course::instance($course->id); // permet d'être dans l'activité voulu
    // Recupere le role et l'id des éléves dans le cours voulu
    $userroles = $DB->get_records('role_assignments', array('contextid' => $context->id));
    // Tous les étudiants permettra de récuperer son nom avec son ID récupérer par $userroles
    $username = $DB->get_records('user');
    $rolenames = role_get_names($context, ROLENAME_ALIAS, true);
    foreach ($userroles as $userrole) {
        // Pour chaque utilisateurs étant tuteur roleid 4 = role non-editing-teacher
        if($userrole->roleid == 4){
            // Verifie le course pour afficher que les evenements de l'activité correspondante
            $hours = sizeof($DB->get_records("univtutoring_events",array('tuteur' => $userrole->userid, 'state' => 4, 'course' => $id)));
            if($hours > 0){
                echo $username[$userrole->userid]->lastname." a effectué ".$hours.get_string('hours',"mod_univtutoring")." <br>";
            }
            // mettre toutes les heures additionnées
        }
    }
}
...
?>
<a href="./view.php?id=<?php echo $id ?>">Calendrier</a>
<?php
echo $OUTPUT->footer();